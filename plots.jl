...
beta = Dist("Bernoulli(.34)", beta_rng(2,8, N), 0, 1)
binomial = Dist("Binomial(.4, 10)", binomial_rng(.43, 10, N), nothing, nothing)
...
mv_norm = Dist("Multivariate Normal([0,2], Σ)", mv_normal_rng([0,2], A, Int(N/100)), nothing, nothing)

for d in [uniform, normal, expon, erlang, gamma, triang, weibull, beta]
    out = plot_dist(d)
    out |> SVG("images/$(d.title).svg", 4inch, 4inch)
end
